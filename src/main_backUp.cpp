...
void findBlob(std::vector<Blob>& currentFrameBlobs, cv::Mat img, cv::Mat imgFrame2, cv::Mat imgFrame2Copy, cv::Mat imgThresh);
    ...
    while (capVideo.isOpened() && chCheckForEscKey != 27) {
		...
		for (uint i = 0; i < p0.size(); i++)
		{
			...
			if (status[i] == 1) {
				...
				circle(frame, p1[i], 20, colors[i], -1);
				//rectangle(frame, p1[i], colors[i]);
			}
		}
		Mat img;
		add(frame, mask, img);
        ...
        for (int i = 0; i < 2; i++) {
            ...
            imgFrame1 = i == 0 ? img : imgFrame1;
            findBlob(tempFrame, imgFrame1, imgFrame2, imgFrame2Copy, imgThresh);
        ...
        for (auto &currentFrameBlobI : farmArr[0]) {
            for (auto &currentFrameBlobJ : farmArr[1]) {
                if (distanceBetweenPoints(currentFrameBlobI.centerPositions.back(), currentFrameBlobJ.centerPositions.back()) <= 50)
				{	
